...
(function ($) {
  ...
  function addHtml() {
    ...
    //前走レースボタン
    keikouUl.append(
      '<li><input type="button" value="前走レース" id="zensou"></li>',
    );
    //前走距離ボタン
    keikouUl.append(
      '<li><input type="button" value="前走距離" id="kyori"></li>',
    );

    var resultTbl = $("<table></table>", {
      id: "resultTbl",
      class: "nk_tb_common race_table_01",
    });

    // 横並び用のコンテナ div を作成
    const $group = $(
      '<div class="checkbox-group" style="display: flex; gap: 15px; margin: 10px 0;"></div>',
    );
  ...
  function bindFunc() {
    $("#kettou").bind("click", function () {
      kaiseki(kettouTyakujunSum);
    });
    $("#wakujun").bind("click", function () {
      kaiseki(wakujunFunc, "cancel");
    });
    $("#zensou").bind("click", function () {
      kaiseki(zensouTyakujunSum);
    });
    $("#kyori").bind("click", function () {
      kaiseki(kyoriTyakujunSum);
    });
  }
  ...
  function makeResultTbl(key, value) {
    ...
    var total =
      Number(tr.find("#1tyaku").text()) +
      Number(tr.find("#2tyaku").text()) +
      Number(tr.find("#3tyaku").text()) +
      Number(tr.find("#4tyaku").text());
    win = (Number(tr.find("#1tyaku").text()) / total).toFixed(2);
    tr.find("#win").text(win);
    ren = (
      (Number(tr.find("#1tyaku").text()) + Number(tr.find("#2tyaku").text())) /
      total
    ).toFixed(2);
